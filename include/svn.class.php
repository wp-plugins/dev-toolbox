	...
	class svnAdmin {
		...
		*/
		...
		function svnAdmin($host, $port=80, $login='', $mdp='') {
			...
			$this->port = $port ; 
			$this->host = $host ; 
			$this->user_agent = "SVN PHP client v1.0 (Sedlex - Wordpress)" ; 
		}
		...
		* Create a new branch
		* 
		* @param string $trunk_fold The trunk folder
		* @param string $branch_fold The new branch folder
		* @param boolean $credentials true if the repository requires credentials 
		* @return array 'isOk' => whether the request is successful,'raw_result' the request and the respond in an array (useful for debugging purpose)
		* @see svnAdmin::getPutFolder	
		*/			
		
		function createBranch($trunk_fold, $branch_fold, $credentials=true) {
			$result = $this->sendSVNRequest($this->host, $trunk_fold, "COPY", "", array("Destination: ".$branch_fold, "Depth: infinity", "Overwrite: T"), $credentials) ; 	
				...
				if ($vcc['isOK']) {
					...
					if ($act['isOK']) {	
						...
						if ($revision['isOK']) {
								...
								if ($res['isOK']) {
									...
									if ($versionFolder['isOK']) {
										...
										if ($urlDepot['isOK']) {
											...
											return array("vcc"=>$vcc['vcc'], "revision"=>$revision['revision'], "versionFolder" => $versionFolder['version_folder'], "putFolder" => $urlDepot['put_folder'], 'activityFolder' => $activity['activity_folder'] , 'uuid' => $uuid , "isOK" => true ) ; 
							...
							return array("isOK" => false, 'step' => 'GETTING THE VERSION', 'raw_result' => $baseline_collection['raw_result']) ; 
						}
					} else {
